...
- **総合的な結論**: 「当たるレースを絞り込む」ことには、**レースクラスを
  未勝利・1勝クラスに絞るという粗い絞り込みには本物の、安定した効果が
  あります**(複勝ROI+2.4pt、6シードで一貫)。ただし、それでもなお平均
  83.4%と黒字には届いておらず、さらに自信度で絞り込む戦略は「時々黒字化
  するが安定しない」という、ギャンブルとしての魅力はあってもシステム的な
  優位性とは言い切れない結果でした。今回の一連の検証を通じて言えるのは、
  このモデルには市場に対するごく小さな優位性(未勝利・1勝クラスでの
  +2〜3pt程度)はあるものの、JRAの控除率(約20%)を安定して覆すほどの
  ものではない、ということです。
