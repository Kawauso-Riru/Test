...
- **総合的な結論**: 一連の絞り込み検証を通じて分かった、再現性のある
  効果は次の2つです。(1) レースクラスを未勝利・1勝クラスに絞る
  (複勝ROI +2〜3pt、標準偏差1.2ptと安定)、(2) モデルの予測1位が
  市場の本命(1番人気)とも一致するレースに絞る(未勝利+1勝クラスとの
  組み合わせで複勝ROI 88.3%、標準偏差2.3pt)。逆に、自信度(calibrated
  probability)だけで絞り込む効果は再現性に乏しく、モデルが市場の人気薄を
  推すケースを狙うのも不安定でした。**それでも88.3%は依然として黒字
  (100%超え)には届いていません** -- JRAの控除率(複勝で約20%)は今回
  見つかったどの組み合わせでも完全には覆せませんでしたが、絞り込みなしの
  81.0%から88.3%まで、統計的に信頼できる形で差を縮められたのは、この
  一連の検証で得られた最も確かな成果です。
