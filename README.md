...
- **総合的な結論**: 一連の絞り込み検証を通じて分かった、再現性のある
  効果は主に3つです。(1) レースクラスを未勝利・1勝クラスに絞る
  (複勝ROI +2〜3pt、標準偏差1.2ptと安定)、(2) モデルの予測1位が
  市場の本命(1番人気)とも一致するレースに絞る(複勝ROI 88.3%、
  標準偏差2.3pt)、(3) さらにその中で自信度上位に絞る(複勝ROI 96.4%、
  n=105)。逆に、自信度単体・市場の人気薄狙いはいずれも再現性に乏しい
  ことも分かりました。**絞り込みなしの81.0%から96.4%まで、階段状に
  着実に差を縮められましたが、最終的に100%(黒字)は超えられません
  でした。** JRAの控除率という壁は、今回試した範囲では完全には崩せ
  ませんでしたが、「どこまで近づけるか」を体系的に検証できたのは
  大きな収穫です。
