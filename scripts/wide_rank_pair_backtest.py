#!/usr/bin/env python
"""Instead of "box the top N picks", does one SPECIFIC pair of predicted
ranks (e.g. rank1+rank3, or rank2+rank4) have a notably better real ワイド
ROI than the others, pooled across multiple held-out splits?

Usage:
    python scripts/wide_rank_pair_backtest.py --seeds 1,2,3,4,5,99
"""
import argparse
import itertools
import sys
...
def evaluate_split(fit_df: pd.DataFrame, feature_columns: list, seed: int, scraper: PoliteScraper, unit: int) -> pd.DataFrame:
    ...
    for race_id, race_rows in valid_df.groupby("race_id"):
        ...
        for i, j in PAIRS:
            ret = pair_return(top[i - 1], top[j - 1], payout, unit)
            row[f"pair_{i}_{j}"] = ret
        rows.append(row)
...
def main() -> None:
    ...
    print(f"\n=== 指数の順位ペア別ワイドROI (n={n} races, {len(seeds)}シードプール, 各ペア{args.unit}円均一) ===")
    results = []
    for i, j in PAIRS:
        ret = combined[f"pair_{i}_{j}"].sum()
        roi = ret / unit_bet * 100
        hit = (combined[f"pair_{i}_{j}"] > 0).sum()
        results.append((i, j, ret, roi, hit))
    results.sort(key=lambda r: r[3], reverse=True)
    print(f"{'rank pair':>12s} {'bet':>10s} {'return':>10s} {'ROI':>8s} {'hits':>6s}")
    for i, j, ret, roi, hit in results:
        print(f"{f'{i}位-{j}位':>12s} {unit_bet:>10d} {ret:>10d} {roi:7.1f}% {hit:>6d}")
