#!/usr/bin/env python
"""Instead of assuming "top-ranked = best combo", does one SPECIFIC triplet
of predicted ranks (e.g. rank2+rank4+rank6, or rank1+rank2+rank3) have a
notably better real 3連複 ROI than the others? Mirrors
wide_rank_pair_backtest.py's approach but for all C(6,3)=20 rank-triplets
among the top 6 picks.

Usage:
    python scripts/fuku3_rank_triplet_backtest.py --seeds 1,2,3,4,5,99
"""
import argparse
import itertools
import sys
...
def main() -> None:
    ...
    for i, j, k in TRIPLETS:
        ...
        results.append((i, j, k, ret, roi, hit))
    results.sort(key=lambda r: r[4], reverse=True)
    print(f"{'rank triplet':>16s} {'bet':>10s} {'return':>10s} {'ROI':>8s} {'hits':>6s}")
    for i, j, k, ret, roi, hit in results:
        print(f"{f'{i}-{j}-{k}位':>16s} {unit_bet:>10d} {ret:>10d} {roi:7.1f}% {hit:>6d}")
